...
var adapter = {
  ...
  create: async function (datastoreName, { newRecord, using: tableName, meta, method }, cb) {
    try {
      ...
      var model = newRecord;
      var insertedRow = results[0];
      var insertedIdKey = insertedRow && Object.keys(insertedRow).find(function (key) {
        return key.toLowerCase() === 'id';
      });

      if (insertedIdKey) {
        model = _.extend({}, newRecord, {
          id: insertedRow[insertedIdKey]
        });
      }
  ...
  join: async function (datastoreName, query, cb) {
    try {
      ...
      if (!registeredDatastores[datastoreName].manager.version) {
        ...
        const versionRows = getNativeRows(report);
        registeredDatastores[datastoreName].manager.version = Number(Object.values(versionRows[0])[0].split('.')[0])
      }
...
function getNativeRows(report) {
  if (report && report.result) {
    if (_.isArray(report.result.rows)) {
      return report.result.rows;
    }

    if (_.isArray(report.result.recordset)) {
      return report.result.recordset;
    }

    if (_.isArray(report.result.recordsets) && _.isArray(report.result.recordsets[0])) {
      return report.result.recordsets[0];
    }
  }

  if (report && _.isArray(report.rows)) {
    return report.rows;
  }

  if (report && _.isArray(report.recordset)) {
    return report.recordset;
  }

  if (report && _.isArray(report.recordsets) && _.isArray(report.recordsets[0])) {
...
function getSchemaName(datastoreName, tableName) {
  ...
  var schemaObject = datastore && datastore.dbSchema && datastore.dbSchema[tableName];

  if (schemaObject && schemaObject.meta && schemaObject.meta.schemaName) {
    return schemaObject.meta.schemaName;
  }

  if (datastore && datastore.config) {
    if (datastore.config.schemaName) {
      return datastore.config.schemaName;
    }

    if (datastore.config.schema) {
